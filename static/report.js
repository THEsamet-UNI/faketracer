...
function initTimelineChart(reportData) {
    ...
    if (!reportData.timelineData || reportData.timelineData.length === 0) {
        ctx.parentElement && (ctx.parentElement.innerHTML = '<div style="padding:20px;color:#94a3b8">Zaman çizelgesi için veri yok</div>');
        return;
    }
    ...
    new Chart(ctx, {
        type: 'line',
        data: { labels, datasets: [{ label: 'Benzerlik Skoru', data, borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.08)', fill: true, tension: 0.4 }] },
        options: {
            responsive: true,
...
function initCountryChart(reportData) {
    ...
    if (Object.keys(stats).length === 0) {
        ctx.parentElement && (ctx.parentElement.innerHTML = '<div style="padding:20px;color:#94a3b8">Ülke dağılımı verisi yok</div>');
        return;
    }
    const labels = Object.keys(stats);
    const data = Object.values(stats);
    const colors = ['#6366f1', '#0ea5e9', '#22c55e', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#14b8a6'];
    new Chart(ctx, { type: 'doughnut', data: { labels, datasets: [{ data, backgroundColor: colors.slice(0, labels.length), borderColor: '#0f172a', borderWidth: 2 }] }, options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: '#0f172a' } } } } });
}
...
function initNetworkGraph(reportData) {
    ...
    if (!container) return;
    try {
        const net = reportData.network || reportData.networkData;
        ...
        const visLib = typeof vis !== 'undefined' ? vis : (typeof visjs !== 'undefined' ? visjs : null);
        if (!visLib) { renderFallbackNetwork(reportData); return; }
        const nodes = new visLib.DataSet(net.nodes.map(node => ({ id: node.id, label: node.label, color: node.group === 'origin' ? '#ef4444' : '#6366f1', font: { color: '#0f172a' } })));
        const edges = new visLib.DataSet((net.edges || []).map(edge => ({ from: edge.from, to: edge.to, color: { color: '#64748b' } })));
        const data = { nodes, edges };
        const options = { physics: { stabilization: true, barnesHut: { gravitationalConstant: -2000, springLength: 100 } }, nodes: { shape: 'dot', size: 20 } };
        new visLib.Network(container, data, options);
    } catch (e) {
        console.error('vis init error', e);
...
function renderFallbackMap(reportData) {
    ...
    list.style.gap = '10px';
    reportData.mapData.forEach(p => {
        const card = document.createElement('div');
        card.style.padding = '12px';
        card.style.background = 'rgba(15,23,42,0.6)';
        card.style.border = '1px solid rgba(99,102,241,0.08)';
        card.style.borderRadius = '8px';
        card.innerHTML = `<strong>${p.name}</strong><br><small>${p.country}</small><div style="margin-top:6px;color:#94a3b8">Benzerlik: ${p.similarity}%</div>`;
...
function renderFallbackTimeline(reportData) {
    ...
    const pad = 30;
    const step = (canvas.width - pad * 2) / (data.length - 1 || 1);
    ...
    data.forEach((v, i) => {
        ...
        ctx.fillText(`N${i + 1}`, x - 8, canvas.height - 8);
    });
    ctx.stroke();
}

function renderFallbackCountry(reportData) {
    ...
    if (keys.length === 0) {
        ...
        if (p) p.innerHTML = '<div style="padding:20px;color:#94a3b8">Ülke dağılımı verisi yok</div>';
        return;
    }
    ...
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const total = keys.reduce((s, k) => s + stats[k], 0) || 1;
    const barH = (canvas.height - 20) / keys.length;
    ctx.font = '12px sans-serif';
    keys.forEach((k, i) => {
        const val = stats[k];
        const w = Math.round((val / total) * (canvas.width - 120));
        const y = 10 + i * barH;
        ctx.fillStyle = '#6366f1';
        ctx.fillRect(100, y + 4, w, barH - 8);
        ctx.fillStyle = '#f1f5f9';
        ctx.fillText(k, 8, y + 14);
        ctx.fillStyle = '#94a3b8';
        ctx.fillText(String(val), 110 + w + 6, y + 14);
    });
}

function renderFallbackNetwork(reportData) {
    const container = document.getElementById('networkGraph');
    if (!container) return;
    const net = reportData.network || reportData.networkData;
    if (!net || !net.nodes || net.nodes.length <= 1) {
    ...
    (net.edges || []).forEach(e => {
        const from = net.nodes.find(x => x.id === e.from);
        const to = net.nodes.find(x => x.id === e.to);
        if (!from || !to) return;
        const fi = net.nodes.indexOf(from), ti = net.nodes.indexOf(to);
        const fx = cx + r * Math.cos(2 * Math.PI * fi / n), fy = cy + r * Math.sin(2 * Math.PI * fi / n);
        const tx = cx + r * Math.cos(2 * Math.PI * ti / n), ty = cy + r * Math.sin(2 * Math.PI * ti / n);
    ...
    net.nodes.forEach((node, i) => {
        const x = cx + r * Math.cos(2 * Math.PI * i / n), y = cy + r * Math.sin(2 * Math.PI * i / n);
        const circle = document.createElementNS(svgNS, 'circle');
        circle.setAttribute('cx', String(x));
        circle.setAttribute('cy', String(y));
        circle.setAttribute('r', '14');
        circle.setAttribute('fill', node.group === 'origin' ? '#ef4444' : '#6366f1');
